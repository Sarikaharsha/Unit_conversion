...
int main() {
  ...
  if(category == 'T'){
   ...
   // Testcases for fahrenheitToCelcius
   int fahrenheitToCelciuss[2][2] = {100,37,200,93};
   
    for (int i = 0; i < n; i++) { 
        c = ((fahrenheitToCelciuss[i][0]-32) * (5.0/9.0)); 
        assert(c == fahrenheitToCelciuss[i][1]); 
      }

 // Testcases for CelciusTofahrenheit
   int CelciusTofahrenheitt[2][2] = {100,212,200,392};
   
    for (int i = 0; i < n; i++) { 
        d = (((9.0/5.0)*CelciusTofahrenheitt[i][0] + 32)); 
        assert(d == CelciusTofahrenheitt[i][1]); 
      }
